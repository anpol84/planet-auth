...
public final class $AuthResponse_JsonReader implements JsonReader<AuthResponse> {
  ...
  public $AuthResponse_JsonReader() {
  }

  private static String read_token(JsonParser __parser, int[] __receivedFields) throws IOException {
    ...
    if (__token == JsonToken.VALUE_STRING) {
      ...
      throw new JsonParseException(__parser, "Expecting [VALUE_STRING, VALUE_NULL] token for field 'token', got " + __token);
    }
  }

  @Override
  @Nullable
  public final AuthResponse read(JsonParser __parser) throws IOException {
    ...
    String token = null;
    if (__parser.nextFieldName(_token_optimized_field_name)) {
      token = read_token(__parser, __receivedFields);
